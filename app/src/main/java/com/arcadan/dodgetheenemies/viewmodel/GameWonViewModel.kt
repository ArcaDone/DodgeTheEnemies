...
class GameWonViewModel : ViewModel() {
    ...
    fun starsShowerAnimation(starAnim: ImageView) {
        viewModelScope.launch {
            delay(300)
            shower(starAnim, AnimationImage.STAR)
        }
    }

    fun rewardsAnimation(
        textView: TextView,
        imageView: ImageView,
        delayMilliSec: Long
    ) {
    ...
    fun playNextLevel(view: View) {
        if (Persistence.instance.getBool(Keys.CONNECTION_STATE)) {
            ...
            if (DataManager.instance.user.value!!.hearts >= GameParameters.instance.deltaHearts) {
                ...
                val selectedLevel = levelSelected + 1
                Persistence.instance.saveInt(Keys.SELECTED_LEVEL, selectedLevel)
                val nextLevel = DataManager.instance.levels[selectedLevel]
                val levelChoose = Gson().toJson(nextLevel)
                val bundle = Bundle()
