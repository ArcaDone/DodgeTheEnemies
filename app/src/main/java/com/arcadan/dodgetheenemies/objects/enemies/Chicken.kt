...
class Chicken constructor(override var sprite: Bitmap, spawnCoordinates: VectorXY) :
    IPhysicalObject {
    ...
    override val hitbox: Rect
        get() = Rect(
            coordinates.x,
            coordinates.y,
            coordinates.x + size.width / 4, coordinates.y + size.height / 4
        )
    ...
    override val shouldBeCleared: Boolean
        get() = if (rightMovement) { hitbox.left > Global.instance.measures.width } else { hitbox.right < 0 }
    ...
    init {
        ...
        coordinates.y = spawnCoordinates.y - size.height
